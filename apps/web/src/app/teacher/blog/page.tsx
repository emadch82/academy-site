...
const EMPTY_FORM = { title: '', excerpt: '', category: '', content: '', imageUrl: '/images/blog2.jpg', readTime: '', status: 'published' as 'published' | 'draft' };
...
export default function TeacherBlogPage() {
  ...
  useMemo(() => {
    ...
    initializeDB();
    setPosts(db.getBlogPostsByAuthor(teacherId));
  ...
  const openEdit = (post: BlogPost) => {
    ...
    setForm({
      ...
      imageUrl: post.imageUrl || '/images/blog2.jpg',
  ...
  const handleSave = () => {
    ...
    if (!form.content.trim()) {
      toast.error('متن مقاله الزامی است');
      return;
    }
    const readTime = form.readTime || `${Math.max(1, Math.ceil(form.content.length / 350))} دقیقه`;
    if (editingId) {
      db.updateBlogPost(editingId, { ...form, readTime });
      ...
      toast.success('مقاله شما در بلاگ منتشر شد');
    }
    setShowModal(false);
    setEditingId(null);
    setForm(EMPTY_FORM);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            مقالاتی که می‌نویسید در صفحه بلاگ سایت نمایش داده می‌شوند
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'مقالات من', value: stats.total, color: 'text-blue-500', bg: 'bg-blue-500/10', icon: FiFileText },
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center py-12">
                    ...
                    <p className="text-lg font-medium">هنوز مقاله‌ای ننوشته‌اید</p>
                    <p className="text-sm text-muted-foreground mt-1">اولین مقاله خود را برای صفحه بلاگ بنویسید</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-background rounded-2xl border p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} rows={2} className="w-full px-3 py-2 rounded-lg border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none" placeholder="خلاصه مقاله" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full px-3 py-2 rounded-lg border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" placeholder="مثلاً: یادگیری زبان" />
                </div>
                <div>
              ...
              <button type="button" onClick={handleSave} className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-medium hover:bg-primary/90 flex items-center justify-center gap-2">
                ...
                {editingId ? 'ذخیره تغییرات' : 'انتشار مقاله'}
              </button>
              <p className="text-xs text-muted-foreground text-center">
                نویسنده: {teacherName} — مقاله بلافاصله در صفحه بلاگ نمایش داده می‌شود
              </p>
            </div>
          </motion.div>
        </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-background rounded-2xl border p-6 w-full max-w-sm mx-4">
